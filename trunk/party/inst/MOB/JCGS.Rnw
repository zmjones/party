...
$O(n)$ operations. For $B > 2$, when an exhaustive search
...
The following \proglang{R} code is sufficient for reproducing the recursively
partitioned model trees from Section~\ref{sec:illustration} as well as their visualizations.
