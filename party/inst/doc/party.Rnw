...
\texttt{ctreecontrol} function, for example
...
ctree(y ~ x1 + x2, data = ls, 
      control = ctreecontrol(teststattype = "quadform"))
@
...
ctreecontrol(savesplitstats = TRUE)
@
...
plot(tptree, terminal_panel = histNode(tptree, breaks = 0:6-0.5, ymax = 65, 
...
sptree <- ctree(arct.lute + pard.lugu + zora.spin + pard.nigr + pard.pull +
           ...
           controls = ctreecontrol(teststat = "maxabs", testtype = "MonteCarlo", nresample = 9999, 
...
plot(sptree, terminal_panel = termNode)
@
