...
stable <- function(i) {
  ...
  paste(i, "&", paste(format(c(0.001, round(x[1,], digits = 3)))[-1], collapse = " & "),
    ...
    format(c(round(p, digits = digits), 0.001))[1])), collapse = " & "), "\\\\", "\\hline", "\n")
}
cat(stable(1))
cat(stable(2))
cat(stable(3))
...
regression by means of estimated coefficients and associated $p$~values. The remaining five columns
summarize the partitioning variables by means of parameter instability statistics and associated
$p$~values.}
\end{center}
\end{table}
...
plot(fmJ, terminal_panel = node_scatterplot(fmJ, col = grey(0.5), linecol = "black"))
@
...
plot(fmPID, terminal_panel = node_bivplot(fmPID, gp = gpar(fill = grey(c(0.5, 0.8))), linecol = "black"))
@
...
\begin{figure}[bth]
\begin{center}
<<GBSG2-plot,echo=FALSE,results=hide,fig=TRUE,height=5,width=8>>=
  ...
  id = TRUE, labels = FALSE)
{
	...
	grid.points(rep.int(x[,2], node$weights), y, gp = gpar(col = col, cex = cex), pch = pch)
        ...
        if(labels) {
            grid.text(xnam[2], x = unit(0.5, "npc"), y = unit(-2, "lines"))
...
plot(fmGBSG2, term = node_gbsg2plot(fmGBSG2, yscale = c(-0.15, 8.7), ylines = 1.5, xscale = c(0, 52), col = grey(0.5), linecol = "black"))
@
...
discuss type II error rates/power

\textbf{Unbiasedness:}
consistency of tests, adjusted for multiple testing
