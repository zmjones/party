...
the general framework from the previous section is applied to various data sets
and models (linear regression, logistic regression, survival regression). The following
subsection are all organized in the same way: In a first
...
more parsimonious are easier to interpret and hence are to be preferred (among those
...
All comparisons are carried out in the framework of \cite{HothornLeischZeileis2005}
...
for performance differences (obtained by treatment contrasts with MOB as the reference level)
...
the binaries available from  \url{http://www.stat.wisc.edu/~loh/} were used.
