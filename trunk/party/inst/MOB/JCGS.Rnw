...
prediction error is compared by root mean squared error (RMSE) differences, 
...
a total of $\Sexpr{(nkBH+1)*ntBH - 1}$ estimated parameters; the associated residual sum of squares is
$\Psi(Y, \hat \theta) = \Sexpr{round(sum(residuals(fmBH)^2), digits = 2)}$,
corresponding to a mean squared error of
\Sexpr{round(mean(residuals(fmBH)^2), digits = 2)}.
