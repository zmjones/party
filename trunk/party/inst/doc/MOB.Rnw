...
\Shorttitle{\pkg{party} with the \code{mob}}
...
The result is the fitted model \code{fm} of class ``\code{mob}'' that contains
...
variable $y$ against each of the regressors $x_i$ in the terminal nodes. As there
are two regressors the \underline{t}erminal \underline{n}ode \underline{ex}tension
\code{tnex} is increased to \code{4}. Each scatter plot also shows the fitted values,
i.e., a project of the fitted hyperplane.
