...
Mac~OS and various flavours of Linux.  It can be installed automatically from within
...
<<ctree>>=
ctGM <- ctree(Class ~ ., data = GM1)
@
creating a fitted \code{BinaryTree} object \code{ctGM}. For obtaining a short summary
of the tree it can be either printed by typing \code{ctGM} at the command line (or more
explicitly: \code{print(ctGM)}) or visualized by \code{plot(ctGM)}. The resulting plot
(depicted in the left panel of Figure~\ref{fig}) shows that the glaucoma probability 
increases clearly for lower eye volumes (as captured in the different volume measures
...
table(true = GM2$Class, pred = predict(ctGM, newdata = GM2))
@
corresponding to a misclassification rate of
\Sexpr{100 * round(1 - sum(diag(table(true = GM2$Class, pred = predict(ctGM, newdata = GM2))))/nrow(GM2), digits = 3)}\%.
...
Similarly, we can fit a whole random forest of \code{ctree}s and compute their confusion
matrix
<<cforest, results=verbatim>>=
cfGM <- cforest(Class ~ ., data = GM1)
table(true = GM2$Class, pred = predict(cfGM, newdata = GM2))
@
corresponding to a misclassification rate of
\Sexpr{format(c(0.1, 100 * round(1 - sum(diag(table(true = GM2$Class, pred = predict(cfGM, newdata = GM2))))/nrow(GM2), digits = 3)))[-1]}\%
Not surprisingly, \code{cforest} improves on \code{ctree}, however the forest does not have
an intuitive visualization like the single tree (see Figure~\ref{fig}).
...
@
Its visualization produced by \code{plot(mobPID)} is depicted in the right panel of
Figure~\ref{fig}. It shows the empirical \code{diabetes} proportions over \code{glucose}
groups along with the predictions from the logisitc regression, conveying that the overall
...
plot(ctGM, newpage = FALSE, pop = TRUE)
popViewport()
pushViewport(viewport(layout.pos.col = 2, layout.pos.row = 1))
plot(mobPID, newpage = FALSE, pop = TRUE, tp_args = list(ylines = 1.3,
  margins = c(1.5, 1, 1.5, 2.2), gp = gpar(fill = grey(c(0.5, 0.8))),
  linecol = "black"))
popViewport(2)
@
...
an additional program layer in \proglang{C} that codes time-critical operations
directly modifying previously created \proglang{R} objects.
