...
### <FIXME> work around reverse = TRUE problem in node_barplot
G <- GM1
G$Class <- factor(as.character(G$Class), levels = c("normal", "glaucoma"))
ctGMtmp <- ctree(Class ~ ., data = G)
### </FIXME>
grid.newpage()
pushViewport(viewport(layout = grid.layout(1, 2)))
pushViewport(viewport(layout.pos.col = 1, layout.pos.row = 1))
plot(ctGMtmp, newpage = FALSE, pop = TRUE)
popViewport()
