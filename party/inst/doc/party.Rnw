...
%%\VignetteSuggests{mvpart,coin,ipred}
...
\begin{quote}
[\ldots] a \textsl{statistical} approach [to recursive partitioning] which
takes
into account the \textsl{distributional} properties of the measures.
\end{quote} 
...
ls <- data.frame(y = gl(3, 50), x1 = rnorm(150) + rep(c(1, 0, 0), c(50, 50,
50)), x2 = runif(150))
@
...
\texttt{ctree.control} function, for example
<<party-teststattype, echo = TRUE, results = hide>>=
ctree(y ~ x1 + x2, data = ls, 
      control = ctree.control(teststattype = "maxabs"))
@
uses $c_\text{max}$ and 
<<party-teststattype2, echo = TRUE, results = hide>>=
ctree(y ~ x1 + x2, data = ls, 
      control = ctree.control(teststattype = "quadform"))
@
...
\end{eqnarray}
The statistics $c(\t_{j^*}^A, \mu_{j^*}^A, \Sigma_{j^*}^A)$ are available
for each node with
<<party-ss, echo = TRUE, results = hide>>=
ctree.control(savesplitstats = TRUE)
@
...
plot(tptree, terminal.panel = histNode(tptree, breaks = 0:6-0.5, ymax = 65, 
...
\begin{figure}[t]
\begin{center}
<<glaucoma-plot, echo = TRUE, fig = TRUE, height = 6, width = 10>>=
plot(gtree)
@
...
\begin{figure}[t]
\begin{center}
<<glaucoma-split-plot, echo = TRUE, fig = TRUE, height = 4, width = 11>>=
...
shown in Figure \ref{glaucoma-boxplot}.

\setkeys{Gin}{width=.5\textwidth}
\begin{figure}[h]
\begin{center}
<<glaucoma-classprob, echo = TRUE, fig = TRUE, height = 4, width = 4>>=
boxplot(sapply(treeresponse(gtree), function(x) x[1]) ~ GlaucomaM$Class, 
        ylab = "Class Probability Glaucoma", ylim = c(0, 1))
@
\caption{Estimated conditional class probabilities for the Glaucoma data.
\label{glaucoma-boxplot}}
\end{center}
\end{figure}
...
sptree <- ctree(arct.lute + pard.lugu + zora.spin + pard.nigr + pard.pull +
           ...
           controls = ctree.control(teststat = "maxabs", testtype = "MonteCarlo", nresample = 9999, 
