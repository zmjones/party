...
\citep[as captured by partitioning,][]{AndrewsPloberger1994}, evaluates all
...
with constant models in the nodes: QUEST \citep{LohShih1997}, the J4.8 implementation 
\citep{WittenFrank2005} of C4.5 \citep{Quinlan1993}, CART and CTree.
...
\citep{R2006}, in particular using the packages \pkg{party} \citep{HothornZeileisHornik2006a},
...
models in the resulting segments---however, there is typically no simple
partition-based model (both with a simple partition and simple local models)
for large data sets.  The algorithm will try to address this
