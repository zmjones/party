...
%% <TH>: data sets klingt immer so nach: haben wir irgendwo gefunden und benutzt
regression problems 
%% </TH>
...
discussion. Using data for $n = 180$ economic journals collected by 
%% <TH> das schaut aus wie eine Referenz Bergstrom, Stock and Watson, da
%% muss man irgendwie trennen
T.~Bergstrom,
\cite{StockWatson2003} 
%% </TH>
fit a demand equation by OLS
...
%% <TH> variablennamen serifenlos wie im plot
in the variables  \textsf{tax} (poperty-tax rate) and \textsf{ptratio} (pupil-teacher ratio).
%% </TH>
