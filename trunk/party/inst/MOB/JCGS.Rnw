...
3 (for GUIDE) or 5 (for RD trees). Furthermore, GUIDE and RA trees assess only the
sign of the residuals, not the full model scores (which can lead to poor power for
certain types of parameter changes, see Section~\ref{sec:discussion}). Finally, RD
...
them into a modern framework for testing structural stability in general parametric
models. This approach makes the evaluation of all conceivable change points feasible,
uses the correct limiting distribution and can be applied to wider class of models.
...
\textbf{Score-based parameter instability tests:}
discuss type II error rates/power (omnibus, not only shift)

\textbf{Unbiasedness:}
size $\alpha$ tests, adjusted for multiple testing, consistent
